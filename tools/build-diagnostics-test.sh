#!/bin/sh
set -eu
c=/usr/local/xscale_be/bin/xscale_be-gcc;s=${1:-/workspace/source};o=${2:-/workspace/build/4vrs-diagnostics-self-test}
"$c" -std=c99 -Os -Wall -Wextra -mcpu=xscale -mbig-endian -msoft-float -I"$s/src" -o "$o" \
 "$s/tests/diagnostics/test_gateway_diagnostics.c" "$s/src/diagnostics/gateway_diagnostics.c" "$s/src/console/gateway_console.c" \
