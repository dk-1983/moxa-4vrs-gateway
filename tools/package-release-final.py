...
host=Path('/workspace/source-host');base=Path('/workspace/build/release-final-20260911');source=base/'source';out=base/'delivery-qualified';out.mkdir(exist_ok=True)
...
archive=out/'4vrs-gateway-v2026.02.01-final-review-candidate.tar.gz'
...
# Pin the exact qualified archive into the delivered helper, before either manifest.
import re
stamped,count=re.subn(rb"(?m)^ARCHIVE_SHA256 = '[0-9a-f]{64}'\r?$",b"ARCHIVE_SHA256 = '"+sha(archive.read_bytes()).encode()+b"'",cf['cf-package.py'])
assert count==1
cf['cf-package.py']=stamped
...
for pattern in ['rng-autonomous-v1*.md','web-ipc-*.md','release-final*.md']:
    for p in (host/'docs').glob(pattern):cf[p.name]=p.read_bytes()
for name in ['moxa2-web-ipc-power-cycles-20260911.ru.md','moxa2-web-ipc-lan-return-20260911.ru.md']:
    cf['forensics/'+name]=(host/'docs/forensics'/name).read_bytes()
...
cf['SHA256SUMS']=''.join(sha(b)+'  '+n+'\n' for n,b in sorted(cf.items())).encode()
zipfiles(out/'4vrs-cf-wizard-v2026.02.01-final-review.zip',cf)
run('delivered-kit',['python3',host/'tools/qualify-delivered-cf-kit.py',out/'4vrs-cf-wizard-v2026.02.01-final-review.zip',out/'delivered-kit.json'])
...
for pattern in ['rng-autonomous-v1*.md','web-ipc-*.md','release-final*.md']:
    for p in (host/'docs').glob(pattern):files[p.relative_to(host).as_posix()]=p.read_bytes()
for p in (host/'docs/releases').glob('v2026.02.01-final-review*.md'):files[p.relative_to(host).as_posix()]=p.read_bytes()
for name in ['moxa2-web-ipc-power-cycles-20260911.ru.md','moxa2-web-ipc-lan-return-20260911.ru.md']:
    files['docs/forensics/'+name]=(host/'docs/forensics'/name).read_bytes()
for name in ['LICENSE','README.md','README.ru.md','THIRD_PARTY.md','THIRD_PARTY.ru.md']:files[name]=(host/name).read_bytes()
zipfiles(out/'4vrs-v2026.02.01-final-review-source-review.zip',files)
