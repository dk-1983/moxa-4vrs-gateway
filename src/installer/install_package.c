...
#include <string.h>
#include <errno.h>
const char *const install_payload_names[4]={"4vrs-install","4vrs-gateway","4vrs-gateway.init","4vrs-networking-wrapper"};
...
void install_package_free(install_package_t*p){unsigned int i;for(i=0;i<4;i++)install_file_free(&p->payload[i]);memset(p,0,sizeof(*p));}
int install_package_read(const char*root,install_package_t*p,const char**stage){
 ...
 for(i=0;i<4;i++){
  if(install_file_read(root,install_payload_names[i],&p->payload[i])||p->payload[i].kind!=1||p->payload[i].mode!=0755||(i<2?elf(&p->payload[i]):script(&p->payload[i])))goto fail;
 ...
 *stage="package-manifest";
 ...
 /* Format 1 deliberately accepts only the canonical maintainer serialization.
 ...
 for(i=0;i<4;i++){
  n=snprintf(expected+at,sizeof(expected)-at,"    {\n      \"mode\": \"0755\",\n      \"name\": \"%s\",\n      \"sha256\": \"%s\",\n      \"size\": %lu\n    }%s\n",install_payload_names[i],p->digest[i],(unsigned long)p->payload[i].size,i==3?"":",");
