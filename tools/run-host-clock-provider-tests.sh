#!/bin/sh
set -eu
root=${1:-.};out=${2:-build/host-clock-provider};mkdir -p "$out"
cc ${CFLAGS:--std=c99 -O2 -Wall -Wextra -Werror} -I"$root/src" -I"$root/tests" -o "$out/test-clock-providers" \
 "$root/tests/app/test_clock_providers.c" "$root/tests/gateway/gateway_orchestration_mock.c" \
 ...
 "$root/src/core/port_runtime.c" "$root/src/core/mock_backend.c" -Wl,--wrap=open,--wrap=close,--wrap=ioctl,--wrap=time,--wrap=clock_settime,--wrap=execl -lrt
"$out/test-clock-providers"
