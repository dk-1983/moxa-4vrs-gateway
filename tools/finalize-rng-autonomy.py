...
base=Path('/workspace/build/rng-autonomous-v1-20260911');host=Path('/workspace/source-host');out=base/'delivery'
sha=lambda b:hashlib.sha256(b).hexdigest()
sourcezip=out/'4vrs-v2026.02.01-rng-autonomous-v1-source-review.zip'
with zipfile.ZipFile(sourcezip) as z:
    files={n:z.read(n) for n in z.namelist() if n!='MANIFEST.json'}
files['tools/finalize-rng-autonomy.py']=(host/'tools/finalize-rng-autonomy.py').read_bytes()
for n in files:
...
for suffix in ['.ru','']:
    ru=bool(suffix)
    for stem in ['rng-autonomous-v1','rng-autonomous-v1-hardware']:
    ...
    text+='- [Gateway installer](4vrs-gateway-v2026.02.01-rng-autonomous-v1-candidate.tar.gz)\n- [CF wizard](4vrs-cf-wizard-v2026.02.01-rng-autonomous-v1.zip)\n- [Source and policy](4vrs-v2026.02.01-rng-autonomous-v1-source-review.zip)\n- [Validation](validation.json) · [Package inventory](review.json) · [SHA256SUMS](SHA256SUMS)\n\n'
