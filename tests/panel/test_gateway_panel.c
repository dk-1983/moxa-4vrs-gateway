...
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;unsigned int draws;memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_BOOTSTRAP);CHECK(gateway_panel_init(&p,&ops,&f)==0);gateway_panel_step(&p,&a);CHECK(has(&f.last,"4VRS Gateway")&&has(&f.last,"v2026.02.01")&&has(&f.last,"0%"));draws=f.draws;gateway_panel_step(&p,&a);CHECK(f.draws==draws&&p.health.suppressed_render_count>0);a.coordinator.state=GATEWAY_APP_READY;a.coordinator.progress_numerator=14;gateway_panel_step(&p,&a);CHECK(p.view==GATEWAY_PANEL_HOME&&has(&f.last,"READY")&&has(&f.last,"F1 Help F3 Menu")&&has(&f.last,"TIME UNSYNC"));gateway_panel_shutdown(&p);CHECK(f.display_closes==1&&f.keypad_closes==1);}
