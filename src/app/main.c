...
static void request_stop(int n){(void)n;if(stop_signal_count<100)++stop_signal_count;if(active_control)active_control->stop_requested=1;}
...
int main(int argc,char**argv){gateway_application_state_t announced=GATEWAY_APP_BOOTSTRAP;const char*directory=GATEWAY_CONFIG_TARGET_DIRECTORY;int console_ready=0;unsigned int final_error=0;if(argc==2&&!strcmp(argv[1],"--network-boot")){const char *stage=0;int result=gateway_network_boot_restore_detailed(gateway_network_environment_production(),&stage);fprintf(stderr,"network-boot %s stage=%s\n",result?"failed":"ok",stage?stage:"unknown");return result?1:0;}
...
#ifdef FOURVRS_WITH_WEB
if(web_gateway_close(&web))final_error=3U;
#endif
if(application.exit_status==GATEWAY_EXIT_CLEANUP_FAILURE)final_error=2U;
/* sync flushes pending OS buffers; it is not an unmount or power-off. */
sync();gateway_panel_stop_result(&panel,final_error);gateway_panel_step(&panel,&application);
if(final_error){sig_atomic_t previous=stop_signal_count;
 while(panel.health.keypad_available&&stop_signal_count==previous){gateway_panel_step(&panel,&application);application.dependencies.wait(application.dependencies.wait_context,GATEWAY_APPLICATION_WAIT_USEC);}}
gateway_panel_shutdown(&panel);active_control=0;return final_error?GATEWAY_EXIT_CLEANUP_FAILURE:(int)gateway_application_exit_status(&application);}
