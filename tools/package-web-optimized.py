"""Full format-3 candidate, strict diff to preserved observer/064851, offline tests."""
...
base_hashes={'064851':'921467d11e52712dd579b8b46c042fe15a18d1c5d3279cc1721732c630712963','observer':'03528b149b6b30fb10ce198d84c0433d570497211aea1bd0a83ef478b18689ae'}
baselines={}
for name,p in base_paths.items():assert sha(p)==base_hashes[name];baselines[name]=unpack(p,out/name)
base=out/'observer';archive=out/'4vrs-gateway-v2026.02.01-web-optimized-candidate.tar.gz'
assert sha(build/'target/4vrs-gateway')==baselines['observer']['4vrs-gateway']['sha256']
...
for a in assets:
 if a['route'] not in ['/app.js','/extended.js','/style.css']:assert a['sha256']==oldassets[a['route']]['sha256']
args=['python3',host/'tools/build-installer-package.py','--version','v2026.02.01']
for flag,name in [('installer','4vrs-install'),('gateway','4vrs-gateway'),('rng','4vrs-rng'),('kdf','4vrs-kdf'),('init-script','4vrs-gateway.init'),('wrapper','4vrs-networking-wrapper'),('license-file','LICENSE.mbedtls')]:args+=['--'+flag,base/name]
...
assert {n for n in records if records[n]!=baselines['observer'][n]}=={'4vrs-web','manifest.json','SHA256SUMS'}
...
for baseline in base_paths:
 runner=(host/'tests/installer/run_release_archive.py').read_text().replace("root/'tests/installer/managed_archive_probe.c'","root/'tests/installer/observer_upgrade_probe.c'")
...
(out/'evidence.json').write_text(json.dumps({'archive':{'name':archive.name,'size':archive.stat().st_size,'sha256':sha(archive)},'files':records,'preserved_baselines':base_hashes,'changed_vs_observer':['4vrs-web','manifest.json','SHA256SUMS'],'assets':assets,'checks':'format3/full inventory/ABI/deterministic/native/public entry/064851+observer upgrade automatic rollback retry no-op return in host+UBSan'},indent=2)+'\n')
