...
cc ${CFLAGS:--std=c99 -O2 -Wall -Wextra -Werror} -I"$root/src" -I"$root/tests" -o "$out/test-gateway-panel" \
 "$root/tests/panel/test_gateway_panel.c" "$root/src/panel/gateway_panel.c" \
