...
int main(int argc,char**argv){binding_policy_roundtrip();if(argc!=2)return 2;if(strcmp(argv[1],"--parser-only")==0){interval_settings();printf("interval parser checks=%u failed=%u word_bits=%u\n",checks,failed,(unsigned)(sizeof(unsigned long)*8U));return failed?1:0;}interval_crc_and_rollback(argv[1]);interval_settings();round_trips();strict_parser_matrix();invalid_typed();filesystem_cases(argv[1]);staged_runtime_contract(argv[1]);product_time_settings();printf("persistence checks=%u failed=%u config=%u max_file=%u max_fds=2 steady_files=2\n",checks,failed,gateway_persistent_config_memory_bytes(),GATEWAY_CONFIG_MAX_BYTES);return failed?1:0;}
