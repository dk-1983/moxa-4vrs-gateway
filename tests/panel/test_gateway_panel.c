...
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;unsigned int draws;memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_BOOTSTRAP);CHECK(gateway_panel_init(&p,&ops,&f)==0);gateway_panel_step(&p,&a);CHECK(has(&f.last,"4VRS Gateway")&&has(&f.last,"v2026.00.01")&&has(&f.last,"0%"));draws=f.draws;gateway_panel_step(&p,&a);CHECK(f.draws==draws&&p.health.suppressed_render_count>0);a.coordinator.state=GATEWAY_APP_READY;a.coordinator.progress_numerator=14;gateway_panel_step(&p,&a);CHECK(p.view==GATEWAY_PANEL_HOME&&has(&f.last,"READY")&&has(&f.last,"F1 Help F3 Menu")&&has(&f.last,"TIME UNSYNC"));gateway_panel_shutdown(&p);CHECK(f.display_closes==1&&f.keypad_closes==1);}
...
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_SAFE_MODE);a.coordinator.error=GATEWAY_APP_ERROR_CONFIG_INVALID;a.coordinator.event_count=1;a.coordinator.events[0].sequence=7;a.coordinator.events[0].stage=GATEWAY_APP_SAFE_MODE;a.coordinator.events[0].progress_percent=100;a.coordinator.events[0].progress_denominator=14;gateway_panel_init(&p,&ops,&f);gateway_panel_step(&p,&a);CHECK(has(&f.last,"SAFE MODE")&&a.coordinator.controller.ports[0].runtime.state==PORT_DISABLED);p.view=GATEWAY_PANEL_DIAGNOSTICS;gateway_panel_step(&p,&a);CHECK(has(&f.last,"Diagnostics"));step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_STARTUP_EVENTS&&has(&f.last,"Sequence 7"));p.view=GATEWAY_PANEL_SYSTEM;p.system_page=1;gateway_panel_step(&p,&a);CHECK(has(&f.last,"Platform:")&&has(&f.last,"ARM XScale")&&has(&f.last,"F5 Network Time"));p.view=GATEWAY_PANEL_ABOUT;gateway_panel_step(&p,&a);CHECK(has(&f.last,"v2026.00.01"));gateway_panel_shutdown(&p);}

static void system_time_view_edit_and_failures(void)
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;unsigned int draws,i;memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_READY);gateway_panel_init(&p,&ops,&f);gateway_panel_step(&p,&a);step(&p,&a,&f,2);p.selected=4U;p.system_page=1U;step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_SYSTEM&&p.system_page==0U&&has(&f.last,"Date & Time:")&&has(&f.last,"2026-09-05")&&has(&f.last,"23:32:15")&&has(&f.last,"Time: Unsynced")&&has(&f.last,"RTC: Unknown")&&has(&f.last,"F5 Set Time")&&has(&f.last,"F4 Platform"));draws=f.draws;gateway_panel_step(&p,&a);CHECK(f.draws==draws);a.time.trust=GATEWAY_TIME_SYNCED;gateway_panel_step(&p,&a);CHECK(has(&f.last,"Time: Synced")&&f.draws==draws+1U);a.time.rtc=GATEWAY_RTC_SAVED;gateway_panel_step(&p,&a);CHECK(has(&f.last,"RTC: Saved"));a.time.rtc=GATEWAY_RTC_WRITE_FAILED;a.time.trust=GATEWAY_TIME_HOLDOVER;gateway_panel_step(&p,&a);CHECK(has(&f.last,"RTC: Write fail")&&has(&f.last,"Time: Holdover"));a.time.trust=GATEWAY_TIME_UNSYNCED;step(&p,&a,&f,3);CHECK(p.system_page==1&&has(&f.last,"Platform:")&&has(&f.last,"ARM XScale")&&has(&f.last,"F5 Network Time")&&has(&f.last,"F2 Date F1 Back"));step(&p,&a,&f,4);CHECK(p.view==GATEWAY_PANEL_NTP_SETTINGS&&has(&f.last,"Network Time:")&&has(&f.last,"Every: 1 h"));step(&p,&a,&f,0);CHECK(p.view==GATEWAY_PANEL_SYSTEM&&p.system_page==1U&&has(&f.last,"Platform:"));step(&p,&a,&f,1);CHECK(p.system_page==0U&&has(&f.last,"Date & Time:"));step(&p,&a,&f,0);CHECK(p.view==GATEWAY_PANEL_MENU);p.selected=4U;p.system_page=1U;step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_SYSTEM&&p.system_page==0U&&has(&f.last,"Date & Time:"));step(&p,&a,&f,4);CHECK(p.view==GATEWAY_PANEL_TIME_EDIT&&p.time_candidate.year==2026U);step(&p,&a,&f,3);CHECK(p.time_candidate.year==2027U);step(&p,&a,&f,0);CHECK(p.view==GATEWAY_PANEL_SYSTEM&&fake_time_sets==0U);step(&p,&a,&f,4);for(i=0;i<6U;++i)step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_TIME_CONFIRM&&fake_time_sets==0U&&has(&f.last,"Set system time?"));step(&p,&a,&f,0);CHECK(p.view==GATEWAY_PANEL_TIME_EDIT&&fake_time_sets==0U);step(&p,&a,&f,2);step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_TIME_RESULT&&fake_time_sets==1U&&has(&f.last,"SYSTEM TIME SET")&&has(&f.last,"RTC NOT SYNCED")&&a.time.trust==GATEWAY_TIME_MANUAL);p.view=GATEWAY_PANEL_TIME_EDIT;p.time_candidate=(gateway_system_time_t){2025U,2U,29U,12U,0U,0U};p.time_field=5;step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_TIME_RESULT&&p.time_result==GATEWAY_SYSTEM_TIME_INVALID&&fake_time_sets==1U);p.view=GATEWAY_PANEL_TIME_CONFIRM;p.time_candidate=(gateway_system_time_t){2024U,2U,29U,12U,0U,0U};fake_time_set_result=GATEWAY_SYSTEM_TIME_SET_FAILED;step(&p,&a,&f,2);CHECK(p.time_result==GATEWAY_SYSTEM_TIME_SET_FAILED&&has(&f.last,"SET FAILED")&&a.process_state==GATEWAY_PROCESS_RUNNING);p.view=GATEWAY_PANEL_TIME_CONFIRM;fake_time_set_result=GATEWAY_SYSTEM_TIME_RTC_SYNC_FAILED;step(&p,&a,&f,2);CHECK(p.time_result==GATEWAY_SYSTEM_TIME_RTC_SYNC_FAILED&&has(&f.last,"RTC SYNC FAILED")&&a.process_state==GATEWAY_PROCESS_RUNNING);CHECK(rows_bounded(&f.last));gateway_panel_shutdown(&p);}
static void editing_confirmation_and_results(void)
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;gateway_port_config_t original;memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_READY);gateway_panel_init(&p,&ops,&f);gateway_panel_step(&p,&a);p.view=GATEWAY_PANEL_CONFIG_PORTS;p.selected=0;p.candidate=a.coordinator.selected;p.candidate_loaded=1;gateway_panel_step(&p,&a);step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_CONFIG_FIELDS);original=p.candidate.ports[0];step(&p,&a,&f,4);CHECK(p.view==GATEWAY_PANEL_EDIT_FIELD);step(&p,&a,&f,3);CHECK(p.candidate.ports[0].enabled!=original.enabled);step(&p,&a,&f,0);CHECK(p.candidate.ports[0].enabled==original.enabled&&p.view==GATEWAY_PANEL_CONFIG_FIELDS);step(&p,&a,&f,4);step(&p,&a,&f,3);step(&p,&a,&f,2);CHECK(p.candidate.ports[0].enabled!=original.enabled&&p.candidate.ports[0].revision==original.revision+1U);p.field_index=10;p.view=GATEWAY_PANEL_CONFIG_FIELDS;gateway_panel_step(&p,&a);step(&p,&a,&f,4);step(&p,&a,&f,3);step(&p,&a,&f,2);CHECK(p.candidate.ports[0].endpoint_port==original.endpoint_port+1U);p.field_index=11;p.view=GATEWAY_PANEL_CONFIG_FIELDS;gateway_panel_step(&p,&a);step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_APPLY_CONFIRM&&p.network_change);step(&p,&a,&f,0);CHECK(p.view==GATEWAY_PANEL_CONFIG_FIELDS);p.candidate=a.coordinator.selected;p.field_index=11;step(&p,&a,&f,2);step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_APPLY_RESULT&&gateway_application_configuration_state(&a)==GATEWAY_CONFIG_TX_UNCHANGED);gateway_panel_step(&p,&a);CHECK(has(&f.last,"NO CHANGES"));a.configuration_transaction.state=GATEWAY_CONFIG_TX_FAILED;a.configuration_transaction.rollback_failed=0;gateway_panel_step(&p,&a);CHECK(has(&f.last,"ROLLED BACK"));a.configuration_transaction.state=GATEWAY_CONFIG_TX_DURABILITY_UNCERTAIN;gateway_panel_step(&p,&a);CHECK(has(&f.last,"DURABILITY?"));gateway_panel_shutdown(&p);}
static void shutdown_confirmation_and_bounds(void)
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;unsigned int r;memset(&f,0,sizeof(f));f.sentinel=0xa55aa55aU;app_fixture(&a,GATEWAY_APP_READY);gateway_panel_init(&p,&ops,&f);gateway_panel_step(&p,&a);p.view=GATEWAY_PANEL_SHUTDOWN_CONFIRM;gateway_panel_step(&p,&a);CHECK(has(&f.last,"Confirm:")&&has(&f.last,"Stop gateway?")&&has(&f.last,"Reboot unavailab"));step(&p,&a,&f,0);CHECK(!a.run_control.stop_requested);p.view=GATEWAY_PANEL_SHUTDOWN_CONFIRM;step(&p,&a,&f,2);CHECK(a.run_control.stop_requested==1);for(r=0;r<8U;++r)CHECK(f.last.row[r][16]=='\0');CHECK(f.sentinel==0xa55aa55aU);CHECK(gateway_panel_screen_bytes()==sizeof(gateway_panel_screen_t));CHECK(gateway_panel_memory_bytes()==sizeof(gateway_panel_t));gateway_panel_shutdown(&p);}

static void title_dictionary_and_bounds(void)
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;unsigned int i;const gateway_panel_view_t views[]={GATEWAY_PANEL_STATUS,GATEWAY_PANEL_PORTS,GATEWAY_PANEL_PORT_DETAIL,GATEWAY_PANEL_CONFIG_PORTS,GATEWAY_PANEL_CONFIG_FIELDS,GATEWAY_PANEL_EDIT_FIELD,GATEWAY_PANEL_APPLY_CONFIRM,GATEWAY_PANEL_APPLY_RESULT,GATEWAY_PANEL_DIAGNOSTICS,GATEWAY_PANEL_STARTUP_EVENTS,GATEWAY_PANEL_SYSTEM,GATEWAY_PANEL_SHUTDOWN_CONFIRM,GATEWAY_PANEL_ABOUT};const char*titles[]={"Status:","Ports:","Port 1:","Config:","Config P1:","Edit Field:","Confirm:","Config Result:","Diagnostics:","Startup Events:","Date & Time:","Confirm:","About:"};memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_READY);gateway_panel_init(&p,&ops,&f);gateway_panel_step(&p,&a);p.candidate=a.coordinator.selected;p.candidate_loaded=1;for(i=0;i<sizeof(views)/sizeof(views[0]);++i){p.view=views[i];gateway_panel_step(&p,&a);CHECK(has(&f.last,titles[i]));CHECK(rows_bounded(&f.last));CHECK(strchr(f.last.row[0],':')!=0);}gateway_panel_shutdown(&p);}

static void fixed_uart_and_local_validation(void)
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;gateway_configuration_transaction_state_t state;memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_READY);gateway_panel_init(&p,&ops,&f);gateway_panel_step(&p,&a);p.view=GATEWAY_PANEL_CONFIG_FIELDS;p.field_index=1;p.port_index=0;p.candidate=a.coordinator.selected;p.candidate_loaded=1;gateway_panel_step(&p,&a);CHECK(has(&f.last,"Fixed at runtime"));step(&p,&a,&f,4);CHECK(p.view==GATEWAY_PANEL_CONFIG_FIELDS);p.candidate.ports[0].enabled=1;p.candidate.ports[1].enabled=1;p.candidate.ports[0].endpoint_port=502;p.candidate.ports[1].endpoint_port=502;p.field_index=11;state=a.configuration_transaction.state;step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_APPLY_RESULT&&p.local_validation_failed==GATEWAY_ERROR_DUPLICATE_ENDPOINT);CHECK(a.configuration_transaction.state==state);CHECK(has(&f.last,"NOT APPLIED"));gateway_panel_shutdown(&p);}
...
{gateway_panel_t p;gateway_application_t a;fake_panel_t f;memset(&f,0,sizeof(f));app_fixture(&a,GATEWAY_APP_READY);a.coordinator.selected.settings.ntp_enabled=1;strcpy(a.coordinator.selected.settings.ntp_server,"10.0.0.10");a.time.ntp_interval_hours=1U;a.time.ntp_enabled=1;strcpy(a.time.ntp_server,"10.0.0.10");a.time.last_ntp_result=GATEWAY_NTP_FAILED;gateway_panel_init(&p,&ops,&f);gateway_panel_step(&p,&a);p.view=GATEWAY_PANEL_SYSTEM;p.system_page=1;gateway_panel_step(&p,&a);CHECK(has(&f.last,"Platform:")&&has(&f.last,"F5 Network Time"));step(&p,&a,&f,4);CHECK(p.view==GATEWAY_PANEL_NTP_SETTINGS&&has(&f.last,"Network Time:")&&has(&f.last,"Every: 1 h")&&has(&f.last,"10.0.0.10")&&has(&f.last,"State: Failed"));step(&p,&a,&f,4);CHECK(p.view==GATEWAY_PANEL_NTP_EDIT);step(&p,&a,&f,2);p.ntp_cursor=7;step(&p,&a,&f,3);CHECK(strcmp(p.candidate.settings.ntp_server,"10.0.0.20")==0);step(&p,&a,&f,0);CHECK(p.view==GATEWAY_PANEL_NTP_SETTINGS&&strcmp(a.coordinator.selected.settings.ntp_server,"10.0.0.10")==0);step(&p,&a,&f,4);step(&p,&a,&f,2);p.ntp_cursor=7;step(&p,&a,&f,3);step(&p,&a,&f,4);CHECK(p.ntp_field==2U&&has(&f.last,"Every: 1 h"));step(&p,&a,&f,3);CHECK(p.candidate.settings.ntp_interval_hours==6U&&has(&f.last,"Every: 6 h"));step(&p,&a,&f,3);CHECK(p.candidate.settings.ntp_interval_hours==24U);step(&p,&a,&f,4);CHECK(p.view==GATEWAY_PANEL_NTP_CONFIRM&&has(&f.last,"10.0.0.20")&&has(&f.last,"Every: 24 h"));step(&p,&a,&f,2);CHECK(p.view==GATEWAY_PANEL_NTP_RESULT&&a.configuration_transaction.state==GATEWAY_CONFIG_TX_ACTIVATING);CHECK(a.process_state==GATEWAY_PROCESS_RUNNING&&a.coordinator.controller.ports[0].lifecycle==GATEWAY_PORT_DISABLED);CHECK(rows_bounded(&f.last));gateway_panel_shutdown(&p);}
