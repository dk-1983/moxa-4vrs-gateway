...
warnings=-Wextra
platform=
if [ "${FOURVRS_PLATFORM:-}" = linux24 ]; then warnings=-W; platform=-DFOURVRS_LINUX24; fi
"$c" -std=c99 -Os -Wall "$warnings" $platform -mcpu=xscale -mbig-endian -msoft-float -I"$root/src" -o "$out" \
