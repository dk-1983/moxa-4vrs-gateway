...
## Ручная установка v2026.00.01

Каждый блок ниже подписан: **компьютер — PowerShell**, **компьютер — Linux**
или **Moxa — SSH**. Выполняйте команды по одной и проверяйте результат перед
следующим шагом. Ненулевой код ошибки означает остановку, а не переход дальше.
...
**Компьютер — PowerShell.** Дополнительно проверьте размер файла:

```powershell
(Get-Item -LiteralPath './4vrs-gateway').Length
```

Ожидаемый результат: `201665`. Проверять контрольную сумму нужно до передачи
и ещё раз после передачи файла на Moxa.
...
`/etc/resolv.conf`, фактическую цель `/etc/rc.d/rcS.d/S40networking`, скрипты
...
**Компьютер — PowerShell.** Укажите текущий IP и SSH-порт своей Moxa:

```powershell
$moxaAddress = Read-Host 'IP Moxa'
$moxaSshPort = [int](Read-Host 'SSH port')
ssh -p $moxaSshPort "root@$moxaAddress"
```

После входа следующие команды выполняются уже **на Moxa — SSH**:
...
Убедитесь, что `id` показывает `uid=0`, `/var/hda` действительно примонтирован
с CompactFlash и накопители доступны. `ls` показывает сетевой загрузочный файл
и, если это ссылка, её назначение. Если путь отсутствует или CF не примонтирована,
не переходите к копированию в системные каталоги: нужно проверить загрузочную
цепочку именно этой системы. Команды выше настройки не меняют.
...
Веб-интерфейс остаётся будущим этапом. Автоматическая установка и обновление
выполняются из пакета по инструкции в начале этого руководства.
