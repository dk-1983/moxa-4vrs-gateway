...
#ifndef INSTALL_ARCHIVE_MAIN
#define INSTALL_ARCHIVE_MAIN main
#endif
int INSTALL_ARCHIVE_MAIN(int argc,char**argv){
 ...
 /* Simulate this archived profile, never another device's hardcoded address. */
 {char directory[1024],data[GATEWAY_NETWORK_SNAPSHOT_MAX];size_t n;gateway_network_profile_t profile;
  CHECK(snprintf(directory,sizeof(directory),"%s/etc/4vrs-network",argv[1])>0);
  CHECK(gateway_network_store_boot(directory,data,sizeof(data),&n)==GATEWAY_NET_STORE_OK);
  CHECK(!gateway_network_profile_decode(data,n,&profile));
  for(i=0;i<2;i++){observation.lan[i].present=observation.lan[i].up=1;
   strcpy(observation.lan[i].address,profile.settings.lan[i].address);strcpy(observation.lan[i].netmask,profile.settings.lan[i].netmask);
   CHECK(!gateway_network_profile_broadcast(&profile,i,observation.lan[i].broadcast));
  }
  observation.default_lan=profile.settings.default_lan;observation.default_routes=!!observation.default_lan;
  if(observation.default_lan)strcpy(observation.gateway,profile.settings.lan[observation.default_lan-1].gateway);
  for(i=0;i<2;i++)strcpy(observation.dns[i],profile.settings.dns[i]);
 }
