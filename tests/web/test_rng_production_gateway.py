...
with tempfile.TemporaryDirectory(prefix='production-gateway-fixture-',dir=build) as tmp:
    for fault in ['quota','corrupt','service','crash']:
        d=Path(tmp)/fault;nv=d/'security/rng';nv.mkdir(parents=True,mode=0o700);(d/'security').chmod(0o700)
        r=subprocess.run([build/'4vrs-rng','production-enable',nv,'--isolated-24h-confirmed'],input=bytes(range(32)),capture_output=True);assert r.returncode==0
        if fault=='quota':
        ...
        with (d/'fixture.log').open('w') as log:
            proc=subprocess.Popen([build/'gateway-host',d,build/'4vrs-web'],stdout=log,stderr=log,env=dict(os.environ,WEB_FIXTURE_HTTP='28080',WEB_FIXTURE_HTTPS='28443'))
            def command(value):
            ...
            try:
                ...
                expected={'quota':37,'corrupt':34,'service':38,'crash':39}[fault]
