...
host=Path('/workspace/source-host');base=Path('/workspace/build/web-ipc-v1-20260911');source=base/'source';out=base/'delivery';out.mkdir(exist_ok=True)
...
archive=out/'4vrs-gateway-v2026.02.01-web-ipc-v1-candidate.tar.gz'
...
for pattern in ['rng-autonomous-v1*.md','web-ipc-*.md']:
...
zipfiles(out/'4vrs-cf-wizard-v2026.02.01-web-ipc-v1.zip',cf)
...
for p in (host/'docs/releases').glob('v2026.02.01-web-ipc-v1*.md'):files[p.relative_to(host).as_posix()]=p.read_bytes()
for name in ['LICENSE','README.md','README.ru.md','THIRD_PARTY.md','THIRD_PARTY.ru.md']:files[name]=(host/name).read_bytes()
zipfiles(out/'4vrs-v2026.02.01-web-ipc-v1-source-review.zip',files)
