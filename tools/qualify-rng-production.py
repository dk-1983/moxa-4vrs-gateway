...
out=Path('/workspace/build/rng-production-v1-20260911')
...
if sys.argv[1]=='build':
    ...
    for mode in ['host','ubsan']:
        ...
        for name in ['test_rng_product','test_rng_metadata','test_rng_production']:
            run(name+'-'+mode,['python3',root/('tests/web/'+name+'.py'),out/mode/'4vrs-rng'])
        run('production-gateway-'+mode,['python3',root/'tests/web/test_rng_production_gateway.py',out/mode])
elif sys.argv[1]=='abi':
    ...
    for mode in ['host','ubsan']:
        run('test_rng_production-'+mode,['python3',root/'tests/web/test_rng_production.py',out/mode/'4vrs-rng'])
