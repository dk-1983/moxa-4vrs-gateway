...
import subprocess,json,hashlib,tarfile,zipfile,os,sys
host=Path('/workspace/source-host');base=Path('/workspace/build/rng-production-v1-20260911');source=base/'source';out=base/'delivery';out.mkdir(exist_ok=True)
...
archive=out/'4vrs-gateway-v2026.02.01-rng-production-v1-candidate.tar.gz'
...
run('package',args+['--output',archive])
run('package-repeat',args+['--output',base/'repeat.tar.gz'])
assert archive.read_bytes()==(base/'repeat.tar.gz').read_bytes()
...
def zipfiles(path,files):
    ...
    with zipfile.ZipFile(path,'w',zipfile.ZIP_DEFLATED) as z:
        for n,b in sorted(files.items()):
            assert not any(x in Path(n).parts for x in ['.git','vendor','state','pending','attempt','start-permit','trial-policy','production-policy','__pycache__'])
...
for suffix in ['','.ru']:
    ...
    cf['rng-production-v1'+suffix+'.md']=(host/('docs/rng-production-v1'+suffix+'.md')).read_bytes()
    cf['rng-production-v1-hardware'+suffix+'.md']=(host/('docs/rng-production-v1-hardware'+suffix+'.md')).read_bytes()
...
zipfiles(out/'4vrs-cf-wizard-v2026.02.01-rng-production-v1.zip',cf)

files={}
for name in ['src','tools','tests','deploy']:
    for p in (host/name).rglob('*'):
        if p.is_file() and '__pycache__' not in p.parts and p.suffix in ['.c','.h','.py','.sh','.js','.css','.html','.json','.md','.conf']:
...
for p in (host/'docs').glob('rng-production-v1*.md'):files[p.relative_to(host).as_posix()]=p.read_bytes()
for p in (host/'docs/releases').glob('v2026.02.01-rng-production-v1*.md'):files[p.relative_to(host).as_posix()]=p.read_bytes()
for name in ['LICENSE','README.md','THIRD_PARTY.md']:files[name]=(host/name).read_bytes()
zipfiles(out/'4vrs-v2026.02.01-rng-production-v1-source-review.zip',files)
...
report={'candidate':'v2026.02.01 rng-production-v1','hardware':'not executed','published':False,'old_candidate_sha256':sha(old.read_bytes()),'inventory':inventory,'source_binary_match':'all compiled src hashes match workspace','policy':'service-only renewal after 24h isolation; one-shot startup permit; 8 committed generations','physical_NAND_write_limit':False}
