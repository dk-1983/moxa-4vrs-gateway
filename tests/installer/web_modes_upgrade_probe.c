...
int main(int argc,char **argv){
 ...
 /* Model the installed console candidates' saved HTTPS configuration.
  * Only public config is generated; no seed/state/admin/TLS fixture here. */
 {gateway_persistent_config_t cfg;install_file_t original;char encoded[GATEWAY_CONFIG_MAX_BYTES];size_t length;
  CHECK(!install_file_read(argv[1],"var/hda/4vrs/config/gateway.conf",&original));
  CHECK(gateway_config_decode((char*)original.data,original.size,&cfg)==GATEWAY_CONFIG_OK);
  cfg.schema_version=2;cfg.settings.web_protocol=1;cfg.settings.web_enabled=1;cfg.settings.web_interface=0;
  if(getenv("WEB_UPGRADE_PROTOCOL")){cfg.schema_version=3;cfg.settings.web_protocol=(unsigned int)atoi(getenv("WEB_UPGRADE_PROTOCOL"));}
  CHECK(gateway_config_encode(&cfg,encoded,sizeof(encoded),&length)==GATEWAY_CONFIG_OK);install_file_free(&original);
  original.kind=1;original.mode=0600;original.data=(unsigned char*)encoded;original.size=length;
  CHECK(!install_file_publish(argv[1],"var/hda/4vrs/config/gateway.conf",&original));
 }
 ...
 printf("saved HTTPS baseline upgrade result=%d stage=%s stops=%u starts=%u\n",r,c.stage,f.stops,f.starts);
 ...
 /* Schema 3 preservation does not qualify an old schema-2 binary's startup. */
 if(getenv("WEB_UPGRADE_PROTOCOL")){
  install_file_free(&config);printf("explicit schema 3 upgrade/rollback/retry/no-op: %u checks PASS\n",checks);return 0;
 }
 ...
 printf("saved HTTPS baseline upgrade/rollback/retry/no-op: %u checks PASS; hardware fake, no RNG process\n",checks);
 return 0;
}
